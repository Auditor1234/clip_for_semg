...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", 
         jit: bool = False, download_root: str = None, classification=False, vis_pretrain=True):
    """Load a CLIP model
    ...
    if not jit:
        model = build_model(state_dict or model.state_dict(), classification=classification, vis_pretrain=vis_pretrain).to(device)
        if str(device) == "cpu":
            model.float()
