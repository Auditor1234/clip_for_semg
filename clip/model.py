...
class VisionTransformer(nn.Module): # 224      32               768         12           12            512
    ...
    def forward(self, x: torch.Tensor):
        x = self.conv1(x)  # shape = [*, width, grid, grid] grid=7
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        x = x + self.positional_embedding.to(x.dtype) # shape(1, 50, 768)
        x = self.ln_pre(x)
...
def build_model(state_dict: dict, classification, vis_pretrain=True):
...
class EMGAttentionPool2d(nn.Module): # 25, 2048,             16,             64
    def __init__(self, spacial_dim: int, embed_dim: int, num_heads: int, output_dim: int = None):
        super().__init__()
...
class EMGModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=400, width=64):
        ...
        self.bn1 = nn.BatchNorm2d(width // 2)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(width // 2, width // 2, kernel_size=(3, 1), padding=(1, 0), bias=False) # shape(B,32,200,1)
        self.bn2 = nn.BatchNorm2d(width // 2)
        self.relu2 = nn.ReLU(inplace=True)
        self.conv3 = nn.Conv2d(width // 2, width, kernel_size=(3, 1), padding=(1, 0), bias=False) # shape(B,64,200,1)
        self.bn3 = nn.BatchNorm2d(width)
        self.relu3 = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        def stem(x):
            ...
            x = self.relu3(self.bn3(self.conv3(x))) # shape(B,64,200,1)
